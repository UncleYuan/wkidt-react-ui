...
class MainCont extends Component {
    ...
    filterCatData = (data) => {
        ...
        newData.unshift({ name: '顶级', value: 0 })
        return newData;
    }
    ...
    eidtColumn = (id) => { //编辑数据
        fetch('oneyuan/category/info/' + id + '.do', {
            method: "get",
            credentials: 'same-origin'
        }).then(response => response.json())
            .then((data) => {
                        ...
                        conf: {
                            footer: (
                                <div>
                                    ...
                                    <a href="javascript:;" onClick={() => { location.hash = "/oneyuan/category.html"; Modal.close(); } } className="btn btn-info">回到列表</a>
                                </div>
                            )
                        }
                    })
