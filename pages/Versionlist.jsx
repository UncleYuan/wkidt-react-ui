...
class AdsCont extends Component {
    ...
    upData = (callback) => { //更新表单数据
        ...
        fetch('/system/versionslist.do?' + tools.parseParam({ versions_number: searchTxt, page: page.page_index, len }), {
            method: "get",
    ...
    sureDelItem = (id) => {
        ...
        fetch('/system/versiondelete.do'+tools.parseParam({id: id instanceof Array?id.join(','):id}), {
            method: "get",
            credentials: 'same-origin',
            headers: {
    ...
    handleAds = (id, status) => {
        Process.show();
        fetch(`system/versionstart.do?` + tools.parseParam({ id: id, status: status==0?1:0 }), {
            method: "get",
            credentials: 'same-origin',
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
    ...
    render() {
        ...
        let cont = rendData.map(function (obj, idx) {
            return (<tr key={idx}>
                ...
                <td>{obj.id}</td>
                <td>{obj.versions_number}</td>
                <td>{obj.description}</td>
                <td>{obj.versions_size}</td>
                <td>{obj.add_time}</td>
                <td>{obj.update_time}</td>
                <td>{obj.start_status}</td>

                <td>

                     <div className="btn-group mb15">
                        <a href={obj.status == 1 ?"javascript:;":("#/system/version/details/edit/" + obj.id)} className={"btn btn-sm "+(obj.status == 1 ?"":"btn-info")}>更新内容</a>
                        {/* <a href="javascript:;" onClick={() => { this.openLogs(obj.id) } } className="btn btn-sm btn-default">审核记录</a>*/}
                        <button className="btn btn-warn" onClick={this.handleAds.bind(this, obj.id, obj.status)} type="button">{obj.status == 1 ? "启用" : "停用"}</button>
                    </div>
                </td>
            </tr>);
        }, this)
                        ...
                        <div className="panel-body">
                            <div className="row">
                                <div className=" col-md-9">
                                    <div className="btn-group mb15">
                                        <a href="#/system/version/details"  className="btn btn-info">添加版本</a>
