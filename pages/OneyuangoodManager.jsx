...
class AdsCont extends Component {
  ...
  upData = (callback) => { //更新表单数据
    ...
    if (this.state.auditType == 'none') {
      ...
      postData = { name: searchTxt, page: page.page_index, len, cat_id: this.state.auditType }
    }
    fetch('oneyuan/goods.do?' + tools.parseParam(postData), {
      method: "get",
      credentials: 'same-origin'
    }).then(response => response.json())
      .then((data) => {
  ...
  render() {
    ...
    let cont = rendData.map(function (obj, idx) {
      return (<tr key={idx}>
        ...
        <td>{obj.name.substr(0, 15)}...</td>
        <td>{obj.price}</td>
        <td>{obj.description.substr(0, 15)}...</td>
        <td>{obj.add_time}</td>
        <td>{obj.merchant_id}</td>
        <td>{obj.is_ready_close==0?'否':'是'}</td>
        <td>

          <div className="btn-group mb15">
            <a href={"#/oneyuan/details/edit/" + obj.id} className="btn btn-sm btn-info">更新商品</a>
              ...
              <div className="row">
                <div className=" col-md-9">
                  ...
                  <div className="inline-block clearfix">
                    <span className="fl pt5 pr10 desalt-color">筛选分类 </span>
                    <div className="fr">
                       <Select className="inline-block" onValueChange={this.handleSelect} options={this.state.categoryData}></Select>
                    </div>
