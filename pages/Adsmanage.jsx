...
class AdsCont extends Component {
  ...
  upData = (callback) => { //更新表单数据
    ...
    fetch('/ads/ads_manage_list.do?' + tools.parseParam(postData), {
      method: "get",
  ...
  sureDelItem = (id) => {
    ...
    fetch('/ads/ads_manage_delete.do?'+tools.parseParam({id: id instanceof Array?id.join(','):id}), {
      method: "get",
      credentials: 'same-origin',
      headers: {
  ...
  render() {
    ...
    let cont = rendData.map(function (obj, idx) {
      return (<tr key={idx}>
        ...
        <td>{obj.add_time}</td>
        <td>{obj.status==0?'已下线':'已发布'}</td>

        <td>

          <div className="btn-group mb15">
            {/*<a href={"#/ads/details/edit/" + obj.id} className="btn btn-sm btn-info">更新广告</a>*/}
