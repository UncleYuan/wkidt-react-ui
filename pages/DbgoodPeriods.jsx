...
class AdsCont extends Component {
    ...
    upData = (callback) => { //更新表单数据
        ...
        fetch(`oneyuan/goods/historydb/${this.props.params.id}.do?` + tools.parseParam(postData), {
            method: "get",
            credentials: 'same-origin'
        }).then(response => response.json())
            .then((data) => {
    ...
    render() {
        ...
        let cont = rendData.map(function (obj, idx) {
            return (<tr key={idx}>
                ...
                <td>{obj.id}</td>
                <td><a href={"#/oneyuan/dbgoods/details/" + obj.id} className="">{obj.name}</a></td>

                <td>{obj.c_renshu}</td>
                <td>{obj.z_renshu}</td>
                <td>{obj.max_periods}</td>
                <td>{obj.cur_periods}</td>
                <td>{obj.price}</td>
                <td>{obj.open_status == 0 ? '否' : '是'}</td>
                <td>{obj.announcetime}</td>
                <td>
