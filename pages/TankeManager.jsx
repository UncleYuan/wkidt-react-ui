...
class MainCont extends Component {
    ...
    upData = (callback) => { //更新表单数据
        ...
        fetch('/explorer/posts.do?' + tools.parseParam(postData), {
            method: "get",
    ...
    sureDelItem = (id) => {
        ...
        fetch('/explorer.do'+tools.parseParam({id: id.join(',')}), {
            method: "get",
            credentials: 'same-origin',
            headers: {
    ...
    subSh = (data) => {
        Process.show();
        fetch(`/explorer/articledown.do?${tools.parseParam({ id: data.id, status: data.status[0] })}`, {
            method: "get",
            credentials: 'same-origin',
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
  ...
  handleSx = (id, status) => {
    ...
    fetch(`/explorer/articledown.do?`+tools.parseParam({ id: id,status:status==1?3:1 }), {
      method: "get",
      credentials: 'same-origin',
      headers: {
        "Content-Type": "application/x-www-form-urlencoded"
    ...
    render() {
        ...
        let cont = rendData.map(function (obj, idx) {
            return (<tr key={idx}>
                ...
                <td>{obj.add_time}</td>
                <td>{obj.publish_status}</td>

                <td>

                    <div className="btn-group mb15">
                    <a href={"#/tanke/details/edit/" + obj.id} className="btn btn-sm btn-info">更新内容</a>
                        {/*<a href={"#/tanke/details/edit/"+obj.id} className="btn btn-sm btn-info">更新内容</a>
                    <a href="javascript:;" onClick={ ()=>{ this.openLogs(obj.id)}} className="btn btn-sm btn-default">审核记录</a>*/}
                       {/* <button className="btn btn-warn" onClick={() => { this.openSh(obj.id) } } type="button">文章下架</button>*/}
                       <a 
                        href="javascript:;" 
                        className="btn btn-sm btn-default" 
                        onClick={this.handleSx.bind(this, obj.id, obj.status)}>{obj.status == 1 ? "下架" : "上线"}</a>
                    </div>
                </td>
            </tr>);
        }, this)
                    ...
                    <section className="panel">
                        ...
                        <div className="panel-body">
                            <div className="row">
                                ...
                                <div className=" col-md-9">
                                    <div className="btn-group mb15">
                                        ...
                                        <a style={{ 'cursor': 'pointer' }} onClick={this.handleAuditType.bind(this, " ")} className={"btn fs11 ml10 mr10 mb5 " + (this.state.auditType == " " ? "btn-info" : "")}>所有</a>
                            ...
                            <div id="column-table">
                                ...
                                <Modal title="管理探客" sizeClass="sm" maxWidth="1000" show={modalShow[1]} onClose={() => { this.toggleModal(1) } }>
