...
class AdsCont extends Component {
  ...
  upData = (callback) => { //更新表单数据
    ...
    if (this.state.auditType == 'none') {
      postData = { title: searchTxt, page: page.page_index, len, cat_id: '' }
    } else {
      postData = { title: searchTxt, page: page.page_index, len, cat_id: this.state.auditType }
    }
    fetch('content/content_list.do?' + tools.parseParam(postData), {
      method: "get",
  ...
  sureDelItem = (id) => {
    ...
    fetch('/content/content_delete.do?' + tools.parseParam({ id: id instanceof Array ? id.join(',') : id }), {
      method: "get",
      credentials: 'same-origin',
      headers: {
  ...
  removeItem = (id) => { //删除数据
    ...
    Modal.show({
      child: <div className="fs14 ">确认是否要删除id为 <span className="base-color">{typeof (id) != "string" ? id.join(',') : id}</span> 的文章？</div>,
      conf: {
        footer: (
      ...
      return (<tr key={idx}>
        ...
        <td>
          ...
          <div className="btn-group mb15">
            <a href={"#/content/article_list/details/edit/" + obj.id} className="btn btn-sm btn-info">更新内容</a>
            {/*<a href="javascript:;" className="btn btn-sm btn-default" onClick={this.handleAds.bind(this, obj.id, obj.status)}>{obj.status == 1 ? "下架" : "上线"}</a>*/}
            ...
            <div className="panel-body">
              <div className="row">
                <div className=" col-md-9">
                  <div className="btn-group mb15">
                    <a href="#/content/article_list/details" className="btn btn-info">添加文章</a>
