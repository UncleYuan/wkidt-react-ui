...
class AdsCont extends Component {
  ...
  upData = (callback) => { //更新表单数据
    ...
    if(this.state.auditType=='none'){
      ...
      postData = { title: searchTxt, page: page.page_index, len ,status:this.state.auditType==0?'0':'1,2'}
    }
    fetch('/jindian/jindian_auditelist.do?' + tools.parseParam(postData), {
      method: "get",
  ...
  sureDelItem = (id) => {
    ...
    fetch('/jindian/jindian_delete.do?'+tools.parseParam({id: id.join(',')}), {
      method: "get",
      credentials: 'same-origin',
      headers: {
  ...
  removeItem = (id) => { //删除数据
    ...
    Modal.show({
      child: <div className="fs14 ">确认是否要删除id为 <span className="base-color">{typeof (id) != "string" ? id.join(',') : id}</span> 的进店有米？</div>,
      conf: {
        footer: (
  ...
  openLogs = (id) => {
    this.openContId = id;
    this.toggleModal(0)
    fetch('/jindian/jindian_audit_log.do?' + tools.parseParam({ content_id: id }), {
      method: "get",
      credentials: 'same-origin'
    }).then(response => response.json())
      .then((data) => {
  ...
  subSh = (data) => {
    Process.show();
    fetch(`/jindian/jindian_audit/${data.id}.do`, {
      method: "put",
      credentials: 'same-origin',
      headers: {
        "Content-Type": "application/x-www-form-urlencoded"
        ...
        <table className="table mt20 table-striped ">
          ...
          <tbody>
            {(() => {
              ...
              return logList[0].map(function (obj, idx) {
                return (<tr key={idx}>
                  <td>{obj.id}</td>
                  ...
                  <td>{obj.add_time}</td>
                  <td>{obj.remark}</td>
                  <td>{obj.jindian_audit_status}</td>
                </tr>);
              }, this)
            })()}
          </tbody>
  ...
  render() {
    ...
    let cont = rendData.map(function (obj, idx) {
      return (<tr key={idx}>
        ...
        <td>{obj.tags}</td>
        <td>{obj.add_time}</td>
        <td>{obj.jindian_status}</td>

        <td>

          <div className="btn-group mb15">
           <a href={"#/jindian/details/edit/"+obj.id} className="btn btn-sm btn-info">详细信息</a>
              ...
              <div className="row">
                <div className=" col-md-9">
                  <div className="btn-group mb15">
                    ...
                    <a style={{ 'cursor': 'pointer' }} onClick={this.handleAuditType.bind(this, "2")} className={"btn fs11 ml10 mr10 mb5 " + (this.state.auditType == "2" ? "btn-info" : "")}>已审核</a>
                  </div>
                </div>
