...
class AdsCont extends Component {
            ...
            .then((data) => {
                if (data.code == "SUCCESS") {
                    ...
                    for (let i = 0; i < data.data.length; i++) {
                        arr.push({ name: data.data[i].name, value: data.data[i].id })
    ...
    upData = (callback) => { //更新表单数据
        ...
        let postData = { page: page.page_index, len }

        fetch(`oneyuan/goods/participated/${this.props.params.id}.do?` + tools.parseParam(postData), {
            method: "get",
            credentials: 'same-origin'
        }).then(response => response.json())
            .then((data) => {
    ...
    toggleModal = (i, str) => { //切换弹窗显示
        let arr = this.state.modalShow;
        arr[i] = !arr[i]
        this.setState({ modalShow: arr, str: str }
        )
    }
        ...
        if (str != undefined) {
            ...
            return (
                <div className="" style={{wordWrap:'break-word' }}>
                    ...
                    {str.split(',').map((obj,idx)=>{
                        return (<span key={idx} className="pr10 pt10">{obj}</span>)
                    })}
    ...
    render() {
        ...
        let cont = rendData.map(function (obj, idx) {
            return (<tr key={idx}>
                ...
                <td>{obj.uid}</td>
                <td><a href={"#/oneyuan/dbgoods/details/" + obj.id} className="">{obj.phone}</a></td>

                <td>{obj.nickname}</td>
                <td>{obj.gon_umber}</td>
                <td><a href="javascript:;" onClick={this.toggleModal.bind(this, 0, obj.gou_code)}>{obj.gou_code.substr(0, 10)}...</a></td>
                <td>{obj.ip}</td>
                <td>{obj.addtime}</td>
                ...
                <td>
                    ...
                    <div className="btn-group mb15">
                        {/* <a href={"#/oneyuan/dbgoods/details/" + obj.id} className="btn btn-sm btn-info">商品详情</a>
                        <a href={"#/oneyuan/dbgoods/periods/" + obj.g_id} className="btn btn-sm btn-default">往期夺宝</a>
                        <a href={"#/oneyuan/dbgoods/record/" + obj.id} className="btn btn-sm btn-warn">参与记录</a>
                        <a href="javascript:;" className="btn btn-sm btn-default" onClick={this.handleAds.bind(this, obj.id, obj.status)}>{obj.status == 1 ? "下架" : "上架"}</a>
            ...
            <div>
                ...
                <div className="wrapper">
                        ...
                        <div className="panel-body">
                            <div className="row">
                                <div className=" col-md-9">
                                    {/* <div className="btn-group mb15">
                    <a href="#/oneyuan/details/edit" className="btn btn-info">添加商品</a>
                    <button className="btn btn-default" onClick={() => { this.removeItem("all") } } type="button">删除所选资源</button>
                            ...
                            <div id="column-table">
                                <Modal title="夺宝号码" maxWidth="1000" show={modalShow[0]} onClose={() => { this.toggleModal(0) } }>
                                    {this.rendlogs()}
                                </Modal>
