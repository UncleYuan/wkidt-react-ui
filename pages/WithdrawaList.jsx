...
class MainCont extends React.Component {
	...
	upData(callback) { //更新表单数据
		...
		fetch('/asset/withdrawal.do?' + tools.parseParam(postData), {
			method: "get",
		...
		const cont = this.state.rendData.map((obj, idx) => {
			...
			return (
				<tr key={idx}>
					...
					<td><span className="base-color fs16">{obj.amount}</span></td>
					<td><span className="base-color fs16">{obj.balance}</span></td>
					<td>{obj.fee}</td>
					<td>{obj.bank_card_no}</td>
					<td>
						{obj.add_time}
					</td>
					<td>
			...
			<div>
				...
				<div className="wrapper">
					<section className="panel">
						<header className="panel-heading">提现记录</header>
