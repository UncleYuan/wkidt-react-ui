...
class AdsCont extends Component {
    ...
    upData = (callback) => { //更新表单数据
        ...
        fetch('merchant/apprecord.do?' + tools.parseParam({ title: searchTxt, page: page.page_index, len, status: this.state.auditType }), {
            method: "get",
    ...
    openLogs = (id) => {
        this.openContId = id;
        this.toggleModal(0)
        fetch(`merchant/oneapprecord.do?${tools.parseParam({ merchant_id: id, page: 1, len: 100 })}`, {
            method: "get",
            credentials: 'same-origin',
        }).then(response => response.json())
            .then((data) => {
                if (data.code == "SUCCESS") {
                    this.setState({ personlogList: data.data })
                } else {
                    this.setState({ personlogList: [] })
                }
            });
    }

    openpersonlogList = (id) => {
        this.toggleModal(0)
    ...
    subSh = (data) => {
        Process.show();
        fetch(`merchant/check/${data.id}/${data.status[0]}.do`, {
            method: "post",
            credentials: 'same-origin',
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
        ...
        return (
            <div className="table-responsive fixed-loading">
                <table className="table mt20 table-striped ">
                    ...
                    <tbody>
                        {(() => {
                            if (personlogList.length == 0) {
                                return (<tr><td className="tc" >暂无记录</td></tr>)
                            }
                            return personlogList.map(function (obj, idx) {
                                return (<tr key={idx}>
                                    <td>{obj.id}</td>
                                    <td>{obj.uid}</td>
    ...
    render() {
        ...
        let cont = rendData.map(function (obj, idx) {
            ...
            return (<tr key={idx}>
                ...
                <td>{obj.id}</td>
                <td><a href={"#/merchant/detailsVerify/edit/" + obj.id} className="">{obj.nickname}</a></td>
                {/*<td onClick={this.openpersonlogList.bind(this, obj.id)}>{obj.nickname}</td>*/}
                ...
                <td>
                    ...
                    <div className="btn-group mb15">
                        {/* <a href={"#/ads/details/edit/"+obj.id} className="btn btn-sm btn-info">更新内容</a>*/}
                        <a href="javascript:;" onClick={() => { this.openLogs(obj.uid) } } className="btn btn-sm btn-default">审核记录</a>
                    ...
                    <section className="panel">
                        ...
                        <div className="panel-body">
                            <div className="row">
                                <div className=" col-md-9">
                                    <div className="btn-group mb15">
                                        ...
                                        <a style={{ 'cursor': 'pointer' }} onClick={this.handleAuditType.bind(this, "0")} className={"btn fs11 ml10 mr10 mb5 " + (this.state.auditType == "0" ? "btn-info" : "")}>待审核</a>
                                        <a style={{ 'cursor': 'pointer' }} onClick={this.handleAuditType.bind(this, "1")} className={"btn fs11 ml10 mr10 mb5 " + (this.state.auditType == "1" ? "btn-info" : "")}>已通过</a>
                                        <a style={{ 'cursor': 'pointer' }} onClick={this.handleAuditType.bind(this, "2")} className={"btn fs11 ml10 mr10 mb5 " + (this.state.auditType == "2" ? "btn-info" : "")}>未通过</a>
                                    </div>
                                </div>
                            ...
                            <div id="column-table">
                                ...
                                <Modal title="查看个人审核记录" maxWidth="1000" show={modalShow[2]} onClose={() => { this.toggleModal(2) } }>
                                    {this.rendpersonlogs()}
                                </Modal>
                                <Modal title="审核商家" sizeClass="sm" maxWidth="1000" show={modalShow[1]} onClose={() => { this.toggleModal(1) } }>
