...
class AdsCont extends Component {
    ...
    getMerchantList = () => {
        let postData = {}
        fetch('merchant/search.do?' + tools.parseParam(postData), {
            method: "get",
    ...
    upData = (callback) => { //更新表单数据
        ...
        if (this.state.auditType == 'none') {
            ...
            postData = { name: searchTxt, page: page.page_index, len, is_recommend: this.state.auditType }
        }
        fetch('/jindian/list.do?' + tools.parseParam(postData), {
            method: "get",
            credentials: 'same-origin'
        }).then(response => response.json())
            .then((data) => {
                        ...
                        <div className="panel-body">
                            <div className="row">
                                <div className=" col-md-9">
                                    <div className="btn-group mb15">
                                        <div className="inline-block  fs12 desalt-color  pb5  mr30">发布状态:</div>
                                        ...
                                        <a style={{ 'cursor': 'pointer' }} onClick={this.handleAuditType.bind(this, "0")} className={"btn fs11 ml10 mr10 mb5 " + (this.state.auditType == "0" ? "btn-info" : "")}>未推荐</a>
                                        <a style={{ 'cursor': 'pointer' }} onClick={this.handleAuditType.bind(this, "1")} className={"btn fs11 ml10 mr10 mb5 " + (this.state.auditType == "1" ? "btn-info" : "")}>已推荐</a>
                                    ...
                                    {/*                                    <div className="inline-block clearfix">
                                        <span className="fl pt5 pr10 desalt-color">分类 </span>
                                        <div className="fr">
                                            <Select search={true} className="inline-block" onValueChange={this.handleSelect} options={this.state.categoryData}></Select>
                                        </div>

                                    </div>
                                    <div className="inline-block clearfix ml10">
                                        <span className="fl pt5 pr10 desalt-color ">商户 </span>
                                        <div className="fr">
                                            <Select search={true} className="inline-block" onValueChange={this.handleSelect} options={this.state.categoryData}></Select>
                                        </div>
