...
class InputEle extends Component {
  ...
  change = (event, t = false) => {
    ...
    let {type, onValueChange, changeType, onUpData} = this.props;
    ...
    if (onUpData && changeType != "upData") { onUpData(value); }
  }
  focus = () => {
    let {onFocusFunc, value, readOnly} = this.props;
    if (readOnly) return;
  ...
  rendInputHtml = () => {
    let {type, placeholder, name, style, readOnly, domDisabled} = this.props;
    ...
    if(domDisabled){ setProps.disabled=true; }
    return createElement(eleType, setProps);
  }
  textareaChange = (dom) => {
