...
class DetailInItem extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  subSh = (data) => {
    ...
    fetch(`/property2/in.do`, {
      ...
      body: tools.parseParam({
        ...
        warehouse_no: data.warehouse_no.join(',')
      })
    }).then(response => response.json())
      .then((data) => {
        Process.Close();
        ...
        if (data.code == "SUCCESS") {
          Modal.show({
            ...
            conf: {
              footer: (
                <a href="javascript:;" onClick={() => { Modal.close(); location.href = "#/dzzc.html/" + proper_id + "/in/" + (new Date()).valueOf() + '/' + item_id; sendUpdataUser(); }} className="btn btn-info">确定</a>)
            }
  ...
  goSubAllSubAll = (data) => {
    ...
    fetch(`/property2/in.do`, {
      ...
      .then((data) => {
        ...
        Toast.show({ msg: data.info });
        if (data.code == "SUCCESS") {
          location.hash = "#/dzzc.html/" + proper_id + "/in/" + (new Date()).valueOf() + '/' + (item_id ? item_id : data.data);
          sendUpdataUser();
        }
      })
  }
  ...
  render() {
    ...
    let isRe = orderInfo.permissions.indexOf('0102') >= 0 && typeof (examineData) !== "undefined" && examineData.status == 0;
    let isSub = orderInfo.permissions.indexOf('0101') >= 0 && !item_id;
    if (loading) {
      ...
      <div>
        <div className="mb5 tl">
          <div className="crumbs">
            <ul>
              <li><a className={selTab == 0 ? "active" : ""} href="javascript:;" onClick={() => { this.turnTab(0); }}>入库申请</a></li>
        ...
        <div className="wrapper">
          ...
          <div className="white-bg" style={{ display: selTab == 0 ? "block" : "none" }}>
            <h4 className="fs20 desalt-color pl30 pt20 pb15">入库申请</h4>
                  ...
                  {(() => {
                    if (orderInfo) {
                      ...
                      if (typeof item_id != "undefined") {
                        let icon = "";
                        let txt = "";
                        let reviewBtn = isRe ? <a href="javascript:;" onClick={() => { this.toggleModal(0); }} className="btn btn-info">入库审核</a> : "";

                        switch (statusNum) {
                          case 0:
                            icon = "icon-shenhezhong assist-color";
                            txt = "已经提交入库申请," + (isRe ? "请审核吧" : "请等待审核吧");
                            break;
                          case 1:
                          ...
                          returnArr.push(
                            <div>
                              <div className="row">
                               ...
                               <div className="row">
                                <div className="col-md-3 desalt-color pb10">已入车库：</div>
                                <div className="col-md-9 pb10">{examineData.warehouse_name}</div>
                              </div>
                              <div className="row">
          ...
          <div style={{ display: selTab == 1 ? "block" : "none" }}>
            <ZcDetailsAuditlog step={"in"} model="property" add_id={item_id} data_id={typeof orderInfo.id == "object" ? orderInfo.id.join('') : orderInfo.id}></ZcDetailsAuditlog>
          </div>
        </div>
      </div>
